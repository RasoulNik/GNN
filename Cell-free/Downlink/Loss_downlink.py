...
class Loss(Layer):
    ...
    #     SINR=tf.zeros([SNR.shape[0],self.Nuser], dtype='float32')
        ...
        num = tf.square(tf.reduce_sum(tf.sqrt(p*SNR), axis=1))
        denom = tf.reduce_sum(SNR*(tf.reduce_sum(p, axis=2,keepdims=True) - p),axis=1)
        SINR = num/denom
        if self.cost_type == 'maxmin':
            ...
            Const = tf.reduce_sum(tf.nn.relu(tf.reduce_sum(p,axis=2)-1),axis=1,keepdims=True)
